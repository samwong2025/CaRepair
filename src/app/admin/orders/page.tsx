...
function resolveStatusFilter(raw: string | string[] | undefined): OrderStatus | 'all' {
  ...
  if (!value) return 'all';
  if (value === 'active' || value === 'completed') return 'all';
  if ((VALID_STATUSES as readonly string[]).includes(value)) return value as OrderStatus;
  return 'all';
}
...
export default async function AdminOrdersPage({
  ...
  return (
    <>
      ...
      <LazyAdmin
        name="orders"
        props={{ orders, currentUser: user, initialStatus, statusPreset }}
      />
    </>
  );
}
