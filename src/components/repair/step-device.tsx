...
export function StepDevice({
  ...
  const models = React.useMemo(() => {
    ...
    return deviceModels
      ...
      .sort((a, b) => Number(Boolean(b.hot)) - Number(Boolean(a.hot)) || b.year - a.year);
  }, [category, keyword]);
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-xl font-extrabold text-ink sm:text-2xl">你想維修邊款裝置？</h2>
        <p className="mt-1.5 text-sm text-ink-muted">
          先揀裝置類別，系統會自動載入對應機型與配件庫存。
        </p>
          ...
          {deviceGroups.map((group) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'group relative flex cursor-pointer flex-col items-start gap-3 rounded-2xl border p-5 text-left transition-all duration-300 ease-smooth',
                  selected
                    ? 'border-brand-500 bg-brand-50/60 shadow-lift'
                    : 'border-slate-200 bg-white hover:-translate-y-1 hover:border-brand-200 hover:shadow-card',
                )}
              >
                {selected ? (
                  <span className="absolute right-3 top-3 flex h-6 w-6 items-center justify-center rounded-full bg-brand-gradient text-white">
