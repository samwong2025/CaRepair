...
export function RepairWizard({ initialCategory }: { initialCategory?: DeviceCategory }) {
  ...
  const validateBooking = (): boolean => {
    ...
    if (form.serviceMode === 'walk_in') {
      ...
      if (!form.district.trim()) next.district = '請填寫地區';
    ...
    if (form.customerEmail && !EMAIL.test(form.customerEmail.trim())) {
  ...
  const canGoNext =
    (step === 1 && Boolean(category && modelId)) ||
    (step === 2 && symptomIds.length > 0) ||
    step === 3 ||
    step === 4;
  ...
  const handleSubmit = async () => {
    ...
    const payload: RepairOrderInput = {
      ...
      district: form.serviceMode === 'walk_in' ? undefined : form.district.trim(),
    ...
    <div ref={topRef} className="scroll-mt-28">
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_20rem] lg:gap-8">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-card sm:p-7 lg:p-8">
          ...
          {step === 3 ? <StepQuote quote={quote} modelName={model?.name ?? ''} /> : null}
          ...
          <div className="mt-8 flex flex-col-reverse gap-3 border-t border-slate-100 pt-6 sm:flex-row sm:items-center sm:justify-between">
            {step > 1 ? (
              <Button variant="ghost" size="lg" onClick={() => goStep(step - 1)} disabled={submitting}>
            ...
            {step < 4 ? (
              ...
              >
                {submitting ? (
                  ...
                  <>
                    確認落單 {quote.total > 0 ? formatHKD(quote.total) : ''}
                    <ArrowRight className="h-4 w-4" />
                  </>
                )}
              </Button>
